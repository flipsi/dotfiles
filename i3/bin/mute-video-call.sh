...
function mute_or_unmute_google_meet() {
    ...
    xdotool search --onlyvisible --name "$WINDOW_NAME_GOOGLE_MEET" \
        windowactivate --sync \
...
function mute_or_unmute_microsoft_teams() {
    ...
    xdotool search --onlyvisible --name "$WINDOW_NAME_MS_TEAMS" \
        windowactivate --sync \
        sleep "$SLEEP_INTERVAL" \
        key ctrl+shift+m
}
...
function mute_or_unmute_slack_huddle() {
    ...
    xdotool search --onlyvisible --name "$WINDOW_NAME_SLACK_HUDDLE" \
        windowactivate --sync \
        sleep "$SLEEP_INTERVAL" \
        key ctrl+shift+space \
    || xdotool search --onlyvisible --name "$WINDOW_NAME_SLACK_MAIN" \
        windowactivate --sync \
        sleep "$SLEEP_INTERVAL" \
        key ctrl+shift+space
}
