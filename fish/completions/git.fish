# copied from https://github.com/petervanderdoes/git-flow-completion/blob/develop/git.fish instead of installed via package manager or install script, since this apparently never updates anyway, so we save a package / installation process.
